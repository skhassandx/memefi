...
| Purchasing TapBot                                              |    ✅     |
| Starting TapBot                                                |    ✅     |
| Claiming TapBot reward every 4 hours                           |    ✅     |
...
| **RANDOM_TAPS**                 | Random number of taps (eg [50,400])                                                                             |
| **RANDOM_TURBO_TAPS**           | Random number of turbo taps (eg [50,400])                                                                       |
| **SLEEP_BETWEEN_REQUESTS**      | Random delay between requests in seconds (eg [10,46])                                                           |
| **DELAY_BETWEEN_TURBO**         | Random delay between turbo taps in seconds (eg [10,46])                                                         |
| **DELAY_BETWEEN_TAPS**          | Random delay between taps in seconds (eg [10,46])                                                               |
| **DELAY_BETWEEN_TASKS**         | Random delay between tasks in seconds (eg [10,46])                                                              |
| **DELAY_BETWEEN_STARTING_BOT**  | Random delay between starting bot (eg [10,46])                                                                  |
...
#Linux and MocOS
~/memefi/4 >>> chmod +x check_node.sh
~/memefi/4 >>> ./check_node.sh

OR

~/memefi/4 >>> npm install
~/memefi/4 >>> cp .env-example .env
~/memefi/4 >>> nano .env # Here you must specify your API_ID and API_HASH , the rest is taken by default
~/memefi/4 >>> node index.js
...
~/memefi/4 >>> npm install
~/memefi/4 >>> cp .env-example .env
~/memefi/4 >>> # Specify your API_ID and API_HASH, the rest is taken by default
~/memefi/4 >>> node index.js
```
...
```shell
~/memefi/4 >>> node index.js --action=1

OR

~/memefi/4 >>> node index.js --action=2 # For session
OR

~/memefi/4 >>> node index.js --action=3 # For query id
